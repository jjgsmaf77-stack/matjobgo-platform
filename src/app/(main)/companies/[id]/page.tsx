      ...
      <div className="bg-white rounded-2xl border p-8">
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            ...
            {company.industry && <span className="text-orange-600 font-medium">{company.industry}</span>}
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">
            채용공고 <span className="text-orange-600">({company.jobPostings.length})</span>
          </h2>
            ...
            <div className="space-y-3">
              {company.jobPostings.map((job) => (
                <Link key={job.id} href={`/jobs/${job.id}`}
                  className="block bg-gray-50 rounded-xl p-5 hover:bg-orange-50 transition">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-bold text-gray-900">{job.title}</h3>
                      <div className="flex items-center gap-3 mt-1 text-sm text-gray-500">
                        <span className="bg-orange-100 text-orange-700 px-2 py-0.5 rounded">{job.field}</span>
                        {job.workLocation && <span>{job.workLocation}</span>}
                        {job.salary && <span className="text-emerald-600">{job.salary}</span>}
                      </div>
                    </div>
                    <span className="text-orange-600 font-medium text-sm">상세보기 →</span>
                  </div>
                </Link>
              ))}
            </div>
